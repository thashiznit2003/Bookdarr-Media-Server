...
- If a browser/OS combo renders lowercase `s` incorrectly in descriptions (looks like words are missing `s`), force a deterministic webfont (see `public/fonts/` and the `BMSMono` `@font-face` in `src/app.service.ts`) rather than relying on system fonts.
